...
def graficar_curva(
    ...
    etiquetas=None,            # Nombres opcionales para cada curva
    estilos_linea=None,         # Estilos opcionales: "-", "--", ":", etc.
...
def aplicar_curva(
    ...
    etiquetas: lista opcional de nombres, uno por canal.
    estilos_linea: lista opcional de estilos, uno por canal.
    """
    ...
    for i, canal in enumerate(channel_grafico):
        ...
        if estilos_linea is not None and i < len(estilos_linea):
            kwargs["linestyle"] = estilos_linea[i]
...
def graficar_bode(
    ...
    """
    Grafica un Bode clásico en una sola figura:
        eje superior -> módulo [dB]
        eje inferior -> fase [grados]

    frecuencia debe estar en Hz y debe ser positiva porque el eje x
    se representa en escala logarítmica.
    """
    frecuencia = np.asarray(
        frecuencia,
        dtype=float
    )
    modulo_db = np.asarray(
        modulo_db,
        dtype=float
    )
    fase_grados = np.asarray(
        fase_grados,
        dtype=float
    )
    ...
    if frecuencia.size == 0:
        raise ValueError(
            "No hay frecuencias positivas válidas para graficar Bode."
        )
    ...
    modulo_db = modulo_db[orden]
    fase_grados = fase_grados[orden]

    fig, (ax_modulo, ax_fase) = plt.subplots(
        2,
        1,
        figsize=(10, 7),
        sharex=True,
